...
export class Cell{
    ...
    setPoppers(){
        if(this.weight === 0 && !this.intialized)
            return;
            ...
            if(Game.config.poppers.head[i].weight == this.weight){
                ...
                this.sprite.on("pointerout", () => {
                  ...
                  this.intialized = true;
                    ...
                    setTimeout( this.replaceSprite.bind(this, this.weight)
                ...
                console.log("space", this.space+((this.position.x+1)*Game.config.grid.cellwidth)+ (Game.config.grid.cellwidth))
                console.log(" ", (Game.config.grid.cellheight)+(this.position.y*Game.config.grid.cellheight)+this.space)
                

                this.sprite.x =((this.position.x+1)*this.space)+((this.position.x+1)*Game.config.grid.cellwidth);//
                this.sprite.y = (this.position.y*Game.config.grid.cellheight)+( (this.position.y+1)*this.space);
    ...
    replaceSprite(weight){
        ...
        if(weight == 0)
            setTimeout(()=>{this.animateTransparent = true;},100);
    ...
    update(deltaTime){
        if(this.animateTransparent)
            this.sprite.alpha =this.sprite.alpha>0? this.sprite.alpha - deltaTime: 0;
